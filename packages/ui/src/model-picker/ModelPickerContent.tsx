...
import { getVirtualizedScrollFadeClassName } from "../scroll-area";
...
import { providerModelKey, sortProviderModelItems } from "./modelOrdering.ts";

/** Persisted favorites supplied by the client that owns this picker. */
export interface ModelPickerPreferences {
  readonly favorites: ReadonlyArray<{
    readonly provider: ProviderInstanceId;
    readonly model: string;
  }>;
  readonly onFavoritesChange: (
    favorites: Array<{ provider: ProviderInstanceId; model: string }>,
  ) => void;
}

/** Client keyboard routing; indices refer to the currently visible selectable models. */
export interface ModelPickerShortcuts {
  readonly labelForIndex: (index: number) => string | null;
  readonly indexForEvent: (event: KeyboardEvent) => number | null;
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  shortcuts?: ModelPickerShortcuts;
  /**
  ...
  const { favorites, onFavoritesChange } = preferences;
  ...
  const modelJumpCommandByKey = useMemo(() => {
    ...
    for (const model of visibleModels) {
      ...
      const jumpCommand = selectableModelIndex;
      if (selectableModelIndex >= MODEL_PICKER_JUMP_KEYBINDING_COMMANDS.length) {
        return mapping;
      }
  ...
  const modelJumpLabelByKey = useMemo((): ReadonlyMap<string, string> => {
    ...
    for (const [modelKey, command] of modelJumpCommandByKey) {
      const label = shortcuts?.labelForIndex(command);
      if (label) {
        mapping.set(modelKey, label);
      }
    }
    return mapping.size > 0 ? mapping : EMPTY_MODEL_JUMP_LABELS;
  }, [shortcuts, modelJumpCommandByKey]);
  ...
  useEffect(() => {
    const onWindowKeyDown = (event: globalThis.KeyboardEvent) => {
      if (event.defaultPrevented || event.repeat || shortcuts?.isSuppressed()) {
        return;
      }

      const jumpIndex = shortcuts?.indexForEvent(event) ?? null;
      if (jumpIndex === null) {
        return;
      }
  ...
  }, [handleModelSelect, shortcuts, modelJumpModelKeys]);
