...
export function useLocalStorage<T, E>(
  ...
  const getSnapshot = useCallback(() => {
    try {
      ...
      Effect.runSync(Console.error("[LOCALSTORAGE] Could not read stored value.", error));
      return null;
    }
  }, [key]);
  ...
  const storedValue = useMemo(() => {
    ...
    try {
      ...
      Effect.runSync(Console.error("[LOCALSTORAGE] Could not decode stored value.", error));
      return initialValue;
    }
  ...
  const setValue = useCallback(
    (value: T | ((val: T) => T)) => {
      try {
        ...
        Effect.runSync(Console.error("[LOCALSTORAGE] Could not update stored value.", error));
      }
    },
